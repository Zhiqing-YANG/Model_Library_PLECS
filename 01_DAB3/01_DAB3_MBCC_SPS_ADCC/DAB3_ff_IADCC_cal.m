...
% ########################################################################
% to calculate phase shift and duty cycle of IADCC (bidirectional)

% Establishment: 28.12.2016 Zhiqing Yang, PGS, RWTH Aachen
% Last change:   28.12.2016 Zhiqing Yang, PGS, RWTH Aachen
    
% ########################################################################
function [d1,d2,dp,mode] = DAB3_ff_IADCC_cal(ff_cal_param,plot_param)
...
P_IADCC_TRI_BUCK_max_op = (2*Ntr^2*pi*Vo^2*(Vi - Ntr*Vo)*(zcs_rel - 1/3)^2)/(L*Vi*w);
P_IADCC_TRI_BOOST_max_op = -(2*pi*Vi^2*(Vi - Ntr*Vo)*(zcs_rel - 1/3)^2)/(L*Ntr*Vo*w);  
P_IADCC_TRAP_max_op = (2*Ntr^2*pi*Vi^2*Vo^2*(3*zcs_rel - 1)^2)/(9*L*w*(Ntr^2*Vo^2 + Ntr*Vi*Vo + Vi^2));
...
P_IADCC_TRI_BUCK_neg_max_op = -(2*Ntr^2*pi*Vo^2*(Vi - Ntr*Vo)*(zcs_rel - 1/3)^2)/(L*Vi*w);
P_IADCC_TRI_BOOST_neg_max_op = (2*pi*Vi^2*(Vi - Ntr*Vo)*(zcs_rel - 1/3)^2)/(L*Ntr*Vo*w);
P_IADCC_TRAP_neg_max_op = -(2*Ntr^2*pi*Vi^2*Vo^2*(3*zcs_rel - 1)^2)/(9*L*w*(Ntr^2*Vo^2 + Ntr*Vi*Vo + Vi^2));

% feedforward parameters calculation
if (Pref >= 0)
    if (Pref < P_IADCC_TRI_BUCK_max_op)
        mode = 'IADCC TRI BUCK';
    ...
    elseif (Pref < P_IADCC_TRI_BOOST_max_op)
        mode = 'IADCC TRI BOOST';
    ...
    elseif (Pref <= P_IADCC_TRAP_max_op)
        mode = 'IADCC TRAP';
        d1 = (Ntr*Vo*(3*(-(8*Vi*(9*L*Pref*w - 2*pi*Ntr^2*Vo^2 + 12*pi*Ntr^2*Vo^2*zcs_rel - 18*pi*Ntr^2*Vo^2*zcs_rel^2 + (9*L*Ntr^2*Pref*Vo^2*w)/Vi^2 + (9*L*Ntr*Pref*Vo*w)/Vi))/(9*Ntr*Vo))^(1/2) + 4*pi^(1/2)*Vi + 4*Ntr*pi^(1/2)*Vo - 12*pi^(1/2)*Vi*zcs_rel - 12*Ntr*pi^(1/2)*Vo*zcs_rel))/(12*Vi*((pi^(1/2)*Ntr^2*Vo^2)/Vi + pi^(1/2)*Ntr*Vo + pi^(1/2)*Vi));
        d2 = d1/(Ntr*Vo/Vi);
        dp = (1/3-zcs_rel)-d2; 
        % deadtime compensation
        if (td_flag == 1)
    ...
    if (Pref >= P_IADCC_TRI_BUCK_neg_max_op) 
        mode = 'IADCC TRI BUCK negative';
        d1 = (2^(1/2)*L^(1/2)*Pref^(1/2)*w^(1/2))/(2*pi^(1/2)*Vi^(1/2)*(Ntr*Vo - Vi)^(1/2));
        d2 = d1/(Ntr*Vo/Vi);
        dp = d1-d2;
        % deadtime compensation
    ...
    elseif (Pref >= P_IADCC_TRI_BOOST_neg_max_op) 
        mode = 'IADCC TRI BOOST negative';
        d1 = -(2^(1/2)*(-L)^(1/2)*Ntr^(1/2)*Pref^(1/2)*Vo^(1/2)*w^(1/2))/(2*pi^(1/2)*Vi*(Ntr*Vo - Vi)^(1/2));
        d2 = d1/(Ntr*Vo/Vi);
        dp = 0;
        % deadtime compensation
    ...
    elseif (Pref >= P_IADCC_TRAP_neg_max_op)
        mode = 'IADCC TRAP negative';
        d1 = (Ntr*Vo*(3*(-(8*Vi*(-9*L*Pref*w - 2*pi*Ntr^2*Vo^2 + 12*pi*Ntr^2*Vo^2*zcs_rel - 18*pi*Ntr^2*Vo^2*zcs_rel^2 - (9*L*Ntr^2*Pref*Vo^2*w)/Vi^2 - (9*L*Ntr*Pref*Vo*w)/Vi))/(9*Ntr*Vo))^(1/2) + 4*pi^(1/2)*Vi + 4*Ntr*pi^(1/2)*Vo - 12*pi^(1/2)*Vi*zcs_rel - 12*Ntr*pi^(1/2)*Vo*zcs_rel))/(12*Vi*((pi^(1/2)*Ntr^2*Vo^2)/Vi + pi^(1/2)*Ntr*Vo + pi^(1/2)*Vi));
        d2 = d1/(Ntr*Vo/Vi);
        dp = d1-(1/3-zcs_rel); 
        % deadtime compensation
...
P_IADCC_TRI_BUCK_max = (2.*Ntr.^2*pi.*Vo_range.^2.*(Vi_range - Ntr.*Vo_range).*(zcs_rel - 1./3).^2)./(L.*Vi_range.*w);
P_IADCC_TRI_BOOST_max = -(2.*pi.*Vi_range.^2.*(Vi_range - Ntr.*Vo_range).*(zcs_rel - 1./3).^2)./(L.*Ntr.*Vo_range.*w); 
P_IADCC_TRAP_max = (2.*Ntr.^2.*pi.*Vi_range.^2.*Vo_range.^2.*(3.*zcs_rel - 1).^2)./(9.*L*w.*(Ntr.^2.*Vo_range.^2 + Ntr.*Vi_range.*Vo_range + Vi_range.^2));
...
P_IADCC_TRI_BUCK_neg_max = -(2.*Ntr.^2*pi.*Vo_range.^2.*(Vi_range - Ntr.*Vo_range).*(zcs_rel - 1./3).^2)./(L.*Vi_range.*w);
P_IADCC_TRI_BOOST_neg_max = (2.*pi.*Vi_range.^2.*(Vi_range - Ntr.*Vo_range).*(zcs_rel - 1./3).^2)./(L.*Ntr.*Vo_range.*w);
P_IADCC_TRAP_neg_max = -(2.*Ntr.^2.*pi.*Vi_range.^2.*Vo_range.^2)./(9.*L.*w.*(Ntr.^2.*Vo_range.^2 + Ntr.*Vi_range.*Vo_range + Vi_range.^2)); 
...
if (Pref >= 0)
    ...
    plot(plot_range(part_1),P_IADCC_TRI_BUCK_max(part_1)/Pn,'Color',col_g,'LineWidth',2);  
    plot(plot_range(part_2),P_IADCC_TRI_BOOST_max(part_2)/Pn,'Color',col_lb,'LineWidth',2);  
    plot(plot_range,P_IADCC_TRAP_max/Pn,'Color',col_b,'LineWidth',2);
    ...
    plot(plot_range(part_1),P_IADCC_TRI_BUCK_neg_max(part_1)/Pn,'Color',col_g,'LineWidth',2);  
    plot(plot_range(part_2),P_IADCC_TRI_BOOST_neg_max(part_2)/Pn,'Color',col_lb,'LineWidth',2);  
    plot(plot_range,P_IADCC_TRAP_neg_max/Pn,'Color',col_b,'LineWidth',2);
...
box on
title('Operation Map');
legend({'Pmax SPS 1','Pmax IADCC Tri-Buck','Pmax IADCC Tri-Boost','Pmax IADCC Trap','SPS ZVS Input Boundary','SPS ZVS Output Boundary'},'Location','northwest');
