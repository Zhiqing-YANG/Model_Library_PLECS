...
enum FSM_State
{
  ...
  FSM_STATE_S9ERROR,
  FSM_STATE_DISTORTION_PREPARE,
  FSM_STATE_DISTORTION_S5ACCKP,
  FSM_STATE_DISTORTION_S4PLLBW
};
...
enum FSM_Transition
{
  ...
  FSM_TRANSITION_DISTORTION_1,
  FSM_TRANSITION_DISTORTION_2,
  FSM_TRANSITION_DISTORTION_3,
  FSM_TRANSITION_DISTORTION_PREPARE_1,
  FSM_TRANSITION_DISTORTION_PREPARE_2,
  FSM_TRANSITION_DISTORTION_S5ACCKP_1,
  FSM_TRANSITION_DISTORTION_S4PLLBW_1
};

#define FSM_MAX_NUM_TAKEN_TRANSITIONS  1
#define FSM_MAX_NUM_TRIGGERS_PER_STATE 4
...
/* input variables */
#define Error                          (*fsm_struct->fsm_inputs[0][0])
#define Start_Up                       (*fsm_struct->fsm_inputs[1][0])
#define Shut_Down                      (*fsm_struct->fsm_inputs[2][0])
#define ACC_On                         (*fsm_struct->fsm_inputs[3][0])
#define ACC_Off                        (*fsm_struct->fsm_inputs[4][0])
#define Distortion_On                  (*fsm_struct->fsm_inputs[5][0])
#define Distortion_Off                 (*fsm_struct->fsm_inputs[6][0])
#define OPTION                         (*fsm_struct->fsm_inputs[7][0])

/* output variables */
#define EN_OLPLL                       (*fsm_struct->fsm_outputs[0][0])
#define EN_ACC                         (*fsm_struct->fsm_outputs[1][0])
#define EN_BW_ACC                      (*fsm_struct->fsm_outputs[2][0])
#define EN_BW_PLL                      (*fsm_struct->fsm_outputs[3][0])
#define SW_Grid                        (*fsm_struct->fsm_outputs[4][0])
#define UNITS0                         (*fsm_struct->fsm_outputs[5][0])
#define UNITS1                         (*fsm_struct->fsm_outputs[6][0])
#define UNITS2                         (*fsm_struct->fsm_outputs[7][0])
#define UNITS3                         (*fsm_struct->fsm_outputs[8][0])
#define State                          (*fsm_struct->fsm_outputs[9][0])
...
static void fsm_state_Distortion_S5ACCKp_EnterAction(const struct FSM_Struct*
  fsm_struct)
{
  EN_OLPLL = 1;
  EN_ACC = 1;
...
static void fsm_state_Distortion_S4PLLBW_EnterAction(const struct FSM_Struct*
  fsm_struct)
{
  EN_OLPLL = 1;
  EN_ACC = 1;
  ...
  if (IsMajorStep) {
    ...
    switch ((int)CurrentState)
    {
     case FSM_STATE_S0IDLE:
      if (Start_Up && !PreviousTriggerValue(0)) {
        ...
        AfterTriggerTimestamp(0) = ceil((0.02) * SamplingFrequency);
     ...
     case FSM_STATE_S3CLACC:
      ...
      } else if (Distortion_On && !PreviousTriggerValue(3)) {
        TakenTransition(0) = FSM_TRANSITION_S3CLACC_4;
        CurrentState = FSM_STATE_DISTORTION_PREPARE;
      }
      break;
     ...
     case FSM_STATE_DISTORTION_PREPARE:
      if (Error && !PreviousTriggerValue(0)) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_1;
        fsm_state_S9Error_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_S9ERROR;
      } else if (!Shut_Down && PreviousTriggerValue(1)) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_2;
        fsm_state_S8ShutDown_EnterAction(fsm_struct);
        AfterTriggerTimestamp(0) = ceil((10) * SamplingFrequency);
        fsm_checkAfterTriggerOverflow(fsm_struct, AfterTriggerTimestamp(0));
        CurrentState = FSM_STATE_S8SHUTDOWN;
      } else if (!Distortion_Off && PreviousTriggerValue(2)) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_3;
        fsm_state_S3CLACC_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_S3CLACC;
      } else if (OPTION == 0) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_PREPARE_1;
        fsm_state_Distortion_S4PLLBW_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_DISTORTION_S4PLLBW;
      } else if (OPTION == 1) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_PREPARE_2;
        fsm_state_Distortion_S5ACCKp_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_DISTORTION_S5ACCKP;
      }
      break;

     case FSM_STATE_DISTORTION_S5ACCKP:
      if (Error && !PreviousTriggerValue(0)) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_1;
        fsm_state_S9Error_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_S9ERROR;
        ...
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_S5ACCKP_1;
        CurrentState = FSM_STATE_DISTORTION_PREPARE;
      }
      break;

     case FSM_STATE_DISTORTION_S4PLLBW:
      if (Error && !PreviousTriggerValue(0)) {
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_1;
        fsm_state_S9Error_EnterAction(fsm_struct);
        CurrentState = FSM_STATE_S9ERROR;
        ...
        TakenTransition(0) = FSM_TRANSITION_DISTORTION_S4PLLBW_1;
        CurrentState = FSM_STATE_DISTORTION_PREPARE;
      }
      break;
    ...
    switch ((int)CurrentState)
    {
     ...
     case FSM_STATE_S3CLACC:
      ...
      PreviousTriggerValue(3) = (Distortion_On);
      break;
     ...
     case FSM_STATE_DISTORTION_PREPARE:
      PreviousTriggerValue(0) = (Error);
      PreviousTriggerValue(1) = (Shut_Down);
      PreviousTriggerValue(2) = (Distortion_Off);
      break;
